...
import com.loopers.core.domain.productlike.ProductLikeFixture;
        ...
        class 사용자가_좋아요한_상품이_존재하는_경우 {
            ...
            void setUp() {
                User user = userRepository.save(
                        UserFixture.createWith("testUser", "test@loopers.com")
                );
                savedUserIdentifier = user.getIdentifier().value();
                savedBrandId = brandRepository.save(BrandFixture.createWith("Test Brand")).getId();
                ...
                productLikeRepository.save(ProductLikeFixture.createWith(user.getId(), product1.getId()));
                productLikeRepository.save(ProductLikeFixture.createWith(user.getId(), product2.getId()));
                productLikeRepository.save(ProductLikeFixture.createWith(user.getId(), product3.getId()));
            }
