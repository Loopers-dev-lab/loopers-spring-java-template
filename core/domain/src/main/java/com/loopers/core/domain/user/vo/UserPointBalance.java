...
import static com.loopers.core.domain.error.DomainErrorCode.USER_POINT_BALANCE_GRATER_THAN_ZERO;

public record UserPointBalance(int value) {

    public UserPointBalance(int value) {
        if (value < 0) {
            throw new IllegalArgumentException(USER_POINT_BALANCE_GRATER_THAN_ZERO.getMessage());
        }
