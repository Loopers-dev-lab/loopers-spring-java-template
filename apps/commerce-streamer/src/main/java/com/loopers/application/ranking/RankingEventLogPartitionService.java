...
/**
 * RankingEventLog 테이블 파티션 관리 서비스
 * 일일 기준으로 파티셔닝하고, 일주일 전 파티션을 자동으로 삭제
 */
@Slf4j
@Service
public class RankingEventLogPartitionService {

    @PersistenceContext
    private EntityManager entityManager;

    private static final String TABLE_NAME = "ranking_event_log";
    ...
    public void createNextDayPartition() {
        ...
        String partitionName = "p_" + tomorrow.format(PARTITION_DATE_FORMATTER);
    ...
    /**
     * 일주일 전 파티션 삭제 (매일 자정에 실행)
     * 파티션이 존재하지 않으면 무시
     */
    @Transactional
    public void dropOldPartitions() {
        LocalDate sevenDaysAgo = LocalDate.now().minusDays(7);
        String partitionName = "p_" + sevenDaysAgo.format(PARTITION_DATE_FORMATTER);
