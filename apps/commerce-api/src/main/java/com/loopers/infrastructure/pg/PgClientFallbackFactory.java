    ...
    return new PgClient() {
      ...
      public PgPaymentResponse requestPayment(String userId, PgPaymentRequest request) {
        throw new PgRequestFailedException(
            "PG 결제 요청 실패 (userId=%s, orderId=%s): %s".formatted(userId, request.orderId(), cause.getMessage()),
            cause
        );
      }
      ...
      public PgTransactionResponse getTransaction(String userId, String transactionKey) {
        throw new PgRequestFailedException(
            "PG 조회 실패 (userId=%s, transactionKey=%s): %s".formatted(userId, transactionKey, cause.getMessage()),
            cause
        );
      }
    };
