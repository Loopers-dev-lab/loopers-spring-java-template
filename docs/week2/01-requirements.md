...
이 명세는 유저 스토리 → 기능 흐름 → 요구사항(기본/추가(TBD)/제약) 순으로 구성되어 있습니다.
...
재고가 0인 상품은 목록에서 제외하거나 별도 표시한다.(필터에서 선택 가능)
...
2. 시스템이 각 상품의 가격을 계산하여 총 결제 금액을 산출한다.
3. 시스템이 재고 확인 및 차감 (부족 시 실패).
4. 시스템이 포인트 잔액을 확인하고 총 결제 금액만큼 차감한다 (부족 시 실패).
5. 성공 시 주문 생성, 실패 시 롤백 (재고/포인트 복구).
6. 예외: 재고/포인트 부족 시 에러 반환.

### 요구사항
#### 기본 요구사항
- 주문 생성 시 재고 차감
- 포인트 차감: 총 결제 금액 확인 후 잔액 차감
- 다중 상품 주문 지원 (items 배열 기반).
