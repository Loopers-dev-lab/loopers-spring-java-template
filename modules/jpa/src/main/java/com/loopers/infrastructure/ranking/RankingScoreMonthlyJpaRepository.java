...
public interface RankingScoreMonthlyJpaRepository extends JpaRepository<RankingScoreMonthly, Long> {

    /**
     * current_score 기준 상위 N개 조회
     */
    @Query("SELECT r FROM RankingScoreMonthly r " +
           "ORDER BY r.currentScore DESC")
    List<RankingScoreMonthly> findTopByOrderByCurrentScoreDesc(int limit);

    /**
     * 배치 UPSERT를 위한 네이티브 쿼리
     */
    @Modifying
    @Query(value = "INSERT INTO ranking_score_monthly " +
