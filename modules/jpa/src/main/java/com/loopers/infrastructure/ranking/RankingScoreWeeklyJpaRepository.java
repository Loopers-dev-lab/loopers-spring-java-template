...
public interface RankingScoreWeeklyJpaRepository extends JpaRepository<RankingScoreWeekly, Long> {

    /**
     * current_score 기준 상위 N개 조회
     */
    @Query("SELECT r FROM RankingScoreWeekly r " +
           "ORDER BY r.currentScore DESC")
    List<RankingScoreWeekly> findTopByOrderByCurrentScoreDesc(int limit);

    /**
     * 배치 UPSERT를 위한 네이티브 쿼리
     */
    @Modifying
    @Query(value = "INSERT INTO ranking_score_weekly " +
